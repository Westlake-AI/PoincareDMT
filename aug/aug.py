...
def aug_near_mix(index, dataset, k=10, random_t=0.1, device="cuda"):
    ...
    if dataset.neighbors_index.device != device:
        dataset.neighbors_index = dataset.neighbors_index.to(device)
    ...
    return random_rate * random_select_near_data2 + (1 - random_rate) * dataset.data[index], random_select_near_index
