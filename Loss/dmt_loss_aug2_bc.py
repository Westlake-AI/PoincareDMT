...
class MyLoss(nn.Module):
    ...
    def forward(
        ...
        #### batch correction
        dis_batch_hot_manifold = self._DistanceSquared(batch_hot[: input_data.shape[0] // 3], metric='euclidean') # 1
        dis_batch_hot_rfa = self._DistanceSquared(batch_hot, metric='euclidean') # 1
        ...
        Q_2 = self._Similarity(
            dist=dis_Q_2 + self.batch_rate * dis_batch_hot_manifold,
        ...
        Q_1 = self._Similarity(
            dist=dis_Q_1 + self.batch_rate * dis_batch_hot_rfa,
